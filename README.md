...
一个简洁优雅的编辑器，提供**真正的所见即所得**写作体验，支持 Markdown、纯文本、图片预览和 Excalidraw 绘图，并可直接管理本地文件夹中的全部文件。
...
### 1. 真正的所见即所得 (WYSIWYG) 编辑

- **即时渲染**：编辑时也能看到渲染效果！
- **字符级实时渲染**：当前光标所在位置显示源码，其他位置实时显示渲染结果
- **智能语法高亮**：光标在格式化文本内时，语法标记（如 `**`、`*`）变暗显示
- **极简界面**：默认隐藏多余的工具栏，提供类似纸张的沉浸式写作体验
- **格式化 Toggle**：选中文本点击格式化按钮可切换添加/移除格式
- **增量式编辑层**：点击编辑时不会“瞬间退回纯源码”，而是保留渲染预览并在下方局部输入，体验更接近 Notion 的无感切换

**工作原理**：
1. 点击任意文本块 → 进入编辑模式
2. 光标所在的格式化文本显示源码（如 `**粗体**`）
3. 光标移开后，格式化文本立即渲染为样式（如 **粗体**）
4. 语法标记始终可见但会变暗，保持编辑的清晰度
...
- 在列表项中按 `Enter` 会智能续写下一条列表（无序/有序/待办）
- 空列表项按 `Enter` 会自然退出列表，不再产生突兀的编辑中断
- 目标是更接近 Notion 风格的“无感编辑”
